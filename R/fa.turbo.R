 ...
 if (!any(fm %in%(c("pa","alpha", "minrank","wls","gls","minres","minchi", "uls","ml","mle","ols" ,"old.min") ))) {message("factor method not specified correctly, minimum residual (unweighted least squares  used")
   fm <- "minres" }
    ...
    switch(cor, 
       ...
       tet = {r <- Turbofuns::PolychoricRM(as.matrix(r), IMissing = 1, NCore = 1)$correlation},
       poly = {r <- Turbofuns::PolychoricRM(as.matrix(r), IMissing = 1, NCore = 1)$correlation},
       tetrachoric = {r <- Turbofuns::PolychoricRM(as.matrix(r), IMissing = 1, NCore = 1)$correlation},
       polychoric = {r <- Turbofuns::PolychoricRM(as.matrix(r), IMissing = 1, NCore = 1)$correlation},
